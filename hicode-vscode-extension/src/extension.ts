...
export function activate(context: vscode.ExtensionContext) {
    console.log('hicode extension is now active!');
    
    // 注册命令
    let generateCode = vscode.commands.registerCommand('hicode.generateCode', async () => {
        try {
    ...
    let completeCode = vscode.commands.registerCommand('hicode.completeCode', async () => {
        try {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                return;
    ...
    let analyzeCode = vscode.commands.registerCommand('hicode.analyzeCode', async () => {
        try {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                return;
    ...
    let collaborateTask = vscode.commands.registerCommand('hicode.collaborateTask', async () => {
        try {
            // 显示协作任务界面
...
async function callHicodeService(endpoint: string, params: any): Promise<any> {
    try {
        ...
        // 实际实现中会发送HTTP请求到本地hicode服务
        // const response = await fetch(`http://localhost:8000/api/v1/hicode/${endpoint}`, {
        //     method: 'POST',
        //     headers: {
