# hicode 对标报告：与 Claude Code / Cursor / Copilot CLI 差距清单

> 版本：hicode v0.4.0（Sprint 1–3 完成后复评）
> 日期：2025
...
经过 Sprint 1–3（运行时修复、CI/CD、错误分类学、兼容层），hicode 已经从"不可运行的原型"升级为
**可测试、可构建、可维护的 agent 编排框架**。但与三大标杆相比，仍存在 **1 个致命级（P0）、8 个高优先（P1）、
8 个中优先（P2）** 差距。

**最核心结论：hicode 目前是一个"没有接入真实 LLM 的 agent 框架"——`llm_call` 仍是返回占位文本的 stub。**
这决定了它和 Claude Code/Cursor/Copilot CLI 之间有一条本质鸿沟：标杆们把"模型能力"作为第一性原理，
hicode 目前只把"编排能力"做出来了。
...
| # | 维度 | hicode 现状 | Claude Code | Cursor | Copilot CLI | hicode |
|---|------|------------|-------------|--------|-------------|--------|
...
| 10 | **开发者体验/CLI** | `hicode run/serve` + headless + TUI（textual，测试内存不稳） | 终端 TUI 标杆级 | 编辑器原生 | 终端 agent | 🟡 |
| 11 | **CI/CD & 质量** | GitHub Actions：ruff format/check → mypy → pytest 3.11/3.12 → build；112 tests | 自家 CI | 自家 CI | 自家 CI | 🟢 |
| 12 | **错误分类学** | `hicode/errors.py`：23 个类型化异常、9 域、severity/code/component/context | 非公开接口 | 非公开接口 | 非公开接口 | 🟢 |
...
| 无结构化错误 | ✅ 已消除 | `hicode/errors.py` 23 异常/9 域；`tests/test_errors.py` 35 例 |
| 死依赖导致运行时崩溃 | ✅ 已消除 | `hicode/compat.py` ~600 行 shim，全模块 import OK |
...
- 修复内容（本次 Sprint）：
  1. 新建 `hicode/llm.py` —— 规范化的多 provider 客户端（OpenAI / Anthropic / DashScope），
     支持非流式完成、**SSE 流式**（两种协议归一化为 OpenAI delta 事件）、工具调用
     （OpenAI-compat + Anthropic Messages）、成本估算、无 key 时优雅回落 stub。
  2. `server/providers.py` 改为 `hicode.llm` 的薄转发（历史 import 路径兼容）。
  3. `hicode/compat.py` 的 `llm_call`/`llm_stream` 从 stub 改为委托 `hicode.llm`。
  4. `server/assembly.py` 删除重复的本地 retry/DEBUG print，接入 compat 的 `retry_with_backoff`。
  5. `config/loader.py` 增加 `llm.provider/model` 配置（读 `HICODE_LLM_PROVIDER`/`HICODE_LLM_MODEL`）。
...
- 新建 `hicode/intent.py`：`IntentClassifier` 双层分类器——
...
- 新建 `hicode/obase/authz.py`（canonical 单源）：
...
**3O 范式工程化（附录 B 落地）**
- 新建 `hicode/obase/` 基础设施层（§2.5 `__manifest__` 7 元素；§7.4 依赖方向强制）；
...
- 扩展端（`.vscode/extensions/hicode-vscode`）：修复 package.json 语法错误（`$(chat)` 缺引号）；
...
- 🎁 连带修复：`_build_tool_schema` 无 docstring 工具越界；test_gate 阻塞事件循环
  （to_thread + HICODE_SKIP_TEST_GATE 守卫防递归）；**发现并清除提交进仓库的真实
  DASHSCOPE_API_KEY**（`.env`/`.hicode.env` git rm --cached + 占位符，旧 key 视为泄露需轮换）。

**G7. 类型安全覆盖极低 → 已修复（解锁 4 个核心文件）**
- mypy 解锁集（CI 阻塞检查）：`server/coordinator.py` + `hicode/streaming.py` +
  `hicode/sandbox.py` + `hicode/intent.py` + obase/llm/errors/compat/utils —— 全部零错误；
  sandbox/obase 开启 `disallow_untyped_defs=true`。
- 修复 numpy stub 3.12 语法问题（`python_version=3.12`，3.11 兼容性由独立 AST 检查把关）；
  CostTracker 单源化（compat 别名 → hicode.utils）；Engine 增加真实 `run_turn`
...
**G12. 多模态仅本地推理占位**
- `hicode/multimodal.py` 存在，但能力边界未与 LLM 集成（无视觉输入到模型）。
...
**G14. 缓存/性能模块缺少真实受益验证**
- `hicode/cache.py`、`performance.py`、`server/routes/performance.py` 功能完整，
...
| 顺序 | 差距 | 工作量 | 前置 |
|------|------|--------|------|
| 1 | **G1 真实 LLM provider** | ✅ 已完成（`hicode/llm.py`） | — |
| 2 | **G2 LLM 意图分类** | ✅ 已完成（`hicode/intent.py`） | G1 ✅ |
| 3 | **G3 遥测 JSONL** | ✅ 已完成（`hicode/obase/telemetry.py`，3O §7） | — |
| 4 | **G4 沙箱 CPU+参数化执行** | ✅ 已完成（ulimit -t + execute_args + 危险前置拦截） | 内存限制已修复 ✅ |
| 5 | **G5 交互式权限确认** | ✅ 已完成（`hicode/obase/authz.py` + CLI/HTTP 接线） | — |
...
> 现状：G1–G9 全部完成，hicode 已具备与标杆"同台竞技"的完整闭环
