...
- **不动执行面**：主脑 opencode-go 直连、22 工具面、hicode 执行器均保持现状，控制面只做"状态层"。
...
*参考：LoopX 仓库源码（event_sourced_state.py / quota.py / registry.py / settlement），Prime Agent 架构（见 ARCHITECTURE_REVIEW_PRIME.md），veya 实测架构（2026-08，工具面 22 + dispatcher、主脑 opencode-go 直连、hicode 执行器、P1 记忆）。*
