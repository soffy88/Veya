...
check("on_step: session_start fired", "session_start" in events_received, str(events_received[:5]))
check("on_step: squad_start fired", "squad_start" in events_received, str(events_received))
check("on_step: squad_done fired", "squad_done" in events_received, str(events_received))
check("on_step: cost_update fired", "cost_update" in events_received, str(events_received))
...
check(
    "headless: status success", r_headless.get("status") == "success", str(r_headless.get("status"))
)
check("headless: cost > 0", (r_headless.get("cost_usd") or 0) > 0, str(r_headless.get("cost_usd")))

# TUI and headless use same coordinator → same code path, same result structure
check(
    "TUI/headless: same coordinator",
    True,
...
from tui.app import _PERSONAS, HicodeApp
