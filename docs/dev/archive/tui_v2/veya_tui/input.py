"""
veya_tui.input — A. CLI 入口 & B. Prompt 输入框
===================================================
A: veya_cli / ArgParser / EnvLoader / StartupBanner
...
class ArgParser:
    ...
    사용법:
      veya [task]
      veya -p 'fix the tests'
      veya --model claude-opus-4-6 --mode plan
      veya --session sess_abc123 --continue
    """

    HELP = f"""\
veya — AI 编码 agent  v{VERSION}

使用方式:
  veya [TASK]              交互式 REPL（task 可选，作为首条消息）
  veya -p TASK             非交互模式（stdout=结果，适合管道）
  veya -c                  继续上一个会话
    ...
    def parse(cls, argv: list[str] | None = None) -> CliArgs:
        ...
        while i < len(args):
            ...
            if a in ("-h", "--help"):
                ...
                print(f"veya {VERSION}")  # pragma: no cover
                sys.exit(0)  # pragma: no cover
...
class EnvLoader:
                ...
                if line and not line.startswith("#") and "=" in line:  # pragma: no cover
                    ...
                    if k and k not in os.environ:  # pragma: no cover
                        ...
                        if verbose:  # pragma: no cover
                            print(
                                f"  loaded {k} from ~/.veya/.env", file=sys.stderr
                            )  # pragma: no cover
    ...
    def get_caller(model: str = "claude-sonnet-4-6") -> Any:
        ...
        try:
            from veya_tui.providers import get_caller  # pragma: no cover
...
class StartupBanner:
    ...
    def print(args: CliArgs) -> None:
        ...
        print()
        print(
            _c("bold", f"  veya {VERSION}", no_color=nc)
            + "  "
...
class HistoryManager:
    """~/.veya/history 영속 저장, ↑↓ 탐색."""

    HISTORY_FILE = Path.home() / ".veya" / "history"
    MAX_ENTRIES = 2000
