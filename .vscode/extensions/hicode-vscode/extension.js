/**
 * hicode VS Code extension — G6 closed loop
 *
...
function activate(context) {
	...
	statusBarItem.text = '$(sparkle) hicode';
	statusBarItem.command = 'hicode.quickChat';
	statusBarItem.show();
	context.subscriptions.push(statusBarItem);

	context.subscriptions.push(
		vscode.commands.registerCommand('hicode.runAgent', () => runAgentCommand()),
		vscode.commands.registerCommand('hicode.debugAgent', () => debugAgentCommand()),
		vscode.commands.registerCommand('hicode.createSession', () => createSessionCommand()),
		vscode.commands.registerCommand('hicode.quickChat', () => quickChatCommand())
	);
}

/** API base URL from settings. */
function apiBase() {
	return vscode.workspace.getConfiguration('hicode').get('apiBaseUrl', 'http://localhost:8000');
}
