...
def _mask_from_id(
    pixels: list[tuple[int, int, int, int]], color: tuple[int, int, int]
) -> list[bool]:
...
def _iou(
    reference_mask: list[bool], render_mask: list[bool], mask: list[bool] | None = None
) -> dict[str, Any]:
    indices = (
        range(len(reference_mask)) if mask is None else (i for i, value in enumerate(mask) if value)
    )
...
def compare_capture(manifest_path: Path, capture_id: str) -> dict[str, Any]:
    ...
    for region in profile.get("regions", []):
        if (
            ...
            or not isinstance(region.get("idColor"), list)
            or len(region["idColor"]) != 3
        ):
            continue
    ...
    if "semantic-id" in loaded:
        ...
        semantic_masks = {
            region: _mask_from_id(reference_semantic, color)
    ...
    for pass_id, (reference_image, render_image) in loaded.items():
        ...
        if pass_id == "alpha-silhouette":
            reference_mask = [
                alpha > 0 and (red or green or blue) > 0
                for red, green, blue, alpha in reference_image[2]
            ]
            render_mask = [
                alpha > 0 and (red or green or blue) > 0
                for red, green, blue, alpha in render_image[2]
            ]
            global_result: dict[str, Any] = _iou(reference_mask, render_mask)
            per_region = {
                region: _iou(reference_mask, render_mask, mask)
                for region, mask in semantic_masks.items()
            }
        elif pass_id == "semantic-id":
            global_result = {
                "pixels": len(pairs),
                "exactMatch": round(sum(ref[:3] == out[:3] for ref, out in pairs) / len(pairs), 6)
                if pairs
                else None,
            }
            per_region = {
                region: {
                    ...
                    "exactMatch": round(
                        sum(
                            ref[:3] == out[:3]
                            for index, (ref, out) in enumerate(pairs)
                            if mask[index]
                        )
                        / sum(mask),
                        6,
            ...
            per_region = {
                region: _similarity(pairs, mask) for region, mask in semantic_masks.items()
            }
    ...
    return {
        ...
        "blockedClaims": []
        if semantic_masks
        else ["per-region scores are unavailable until semantic-id pass has region colors"],
    }
