...
def mesh_edge_counts(
    vertices: list[Any], indices: list[Any], *, precision: int = 6
) -> dict[str, int]:
    ...
    triangles: list[Any] = (
        ...
        else [indices[i : i + 3] for i in range(0, len(indices), 3)]
    )
...
def _bounds(mesh: dict[str, Any]) -> list[list[float]] | None:
    ...
    if (
        ...
        and len(bounds) == 2
        and all(isinstance(point, (list, tuple)) and len(point) >= 3 for point in bounds)
    ):
...
def _axis_index(value: Any) -> int:
    if isinstance(value, int) and value in (0, 1, 2):
        return value
    return {"x": 0, "y": 1, "z": 2, "long": 0, "length": 0, "thickness": 2}.get(
        str(value or "").lower(), 0
    )
...
def _blade_thickness(mesh: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "grindVariation": grind_variation,
        "distalVariation": distal_variation,
        "constantGrind": grind_variation <= 1e-6,
        "missingDistalTaper": distal_variation <= 1e-6,
        "reported": bool(values),
    }


def _triangle_count(mesh: dict[str, Any]) -> int | None:
    if isinstance(mesh.get("triangleCount"), (int, float)) and not isinstance(
        mesh["triangleCount"], bool
    ):
        return max(0, int(mesh["triangleCount"]))
    indices = mesh.get("indices")
    if isinstance(indices, list):
        return (
            len(indices) if indices and isinstance(indices[0], (list, tuple)) else len(indices) // 3
        )
    return None
...
def _normal_consistency(mesh: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if len(normals) != len(mesh["vertices"]):
        return {
            "checked": False,
            "consistent": False,
            "reason": "normal count does not match vertex count",
        }
    indices = mesh.get("indices")
    triangles = (
        indices
        if isinstance(indices, list) and indices and isinstance(indices[0], (list, tuple))
        else (
...
def measure_geometry_integrity(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    performance = (
        payload.get("performanceBudget")
        if isinstance(payload.get("performanceBudget"), dict)
        else {}
    )
    triangle_budget = performance.get(
        "triangleBudget", payload.get("triangleBudget", DEFAULT_TRIANGLE_BUDGET)
    )
    if (
        not isinstance(triangle_budget, (int, float))
        or isinstance(triangle_budget, bool)
        or triangle_budget <= 0
    ):
    ...
    for index, mesh in enumerate(meshes):
        ...
        item: dict[str, Any] = {
            ...
            "realization": mesh.get("realization", "separate-geometry"),
        }
            ...
            item.update(
                {
                    ...
                    "note": "mesh topology not supplied",
                }
            )
        ...
        if normals is not None:
            ...
            failures.append(message)
            item["normalConsistency"] = {
                "checked": False,
                "consistent": False,
                "reason": "normals missing",
            }
            issues.append(
                {
                    "severity": "error",
        ...
        if role == "blade" or "blade" in str(item["id"]).lower():
            ...
            if item["thickness"]["constantGrind"] or item["thickness"]["missingDistalTaper"]:
                failures.append(
                    f"{item['id']}: blade has constant thickness or missing distal taper"
                )
        if item.get("boundaryEdges", 0) not in (0, None):
            message = f"{item['id']}: openEdges={item['boundaryEdges']} ({item['realization']})"
            failures.append(message)
            issues.append(
                {"severity": "error", "code": "watertight", "mesh": item["id"], "message": message}
            )
    ...
    for first, second in combinations([item for item in meshes if isinstance(item, dict)], 2):
        ...
        attached = isinstance(second.get("attachment"), dict) and second["attachment"].get(
            "parentId"
        ) == first.get("id")
        ...
        seam = {
            ...
            "minimum": SEAM_MIN_OVERLAP,
            "source": SEAM_SOURCE,
        }
        seams.append(seam)
        if overlap < SEAM_MIN_OVERLAP:
            failures.append(
                f"seam {seam['a']}↔{seam['b']}: overlap={overlap:.3f} < {SEAM_MIN_OVERLAP:.2f}"
            )
    if total_triangles > triangle_budget:
        ...
        if not lod_plan_valid:
            ...
            code = (
                "lod-required" if not isinstance(lod_plan, list) or not lod_plan else "lod-invalid"
            )
            issues.append({"severity": "error", "code": code, "message": lod_message})
    return {
        "passed": not failures,
        "source": SEAM_SOURCE,
        "minimumSeamOverlap": SEAM_MIN_OVERLAP,
        "maximumDocumentedSeamOverlap": SEAM_MAX_OVERLAP,
        "triangleBudget": int(triangle_budget),
        "triangleCount": total_triangles,
        "lodPlanPresent": isinstance(lod_plan, list) and bool(lod_plan),
