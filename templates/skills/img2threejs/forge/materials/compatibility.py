...
def check_compatibility(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    for component in components:
        ...
        if material_id:
            ...
            if str(material_id) not in materials:
                failures.append(
                    f"component {component_id!r} references missing material {material_id!r}"
                )
        if component.get("materialRegions") and not material_id:
            failures.append(
                f"component {component_id!r} has materialRegions but no material binding"
            )
    for material_id, material in materials.items():
        ...
        has_uv_contract = isinstance(projection, dict) or any(
            ...
            for component in components
            if component.get("material") == material_id
        )
        if required_maps and not has_uv_contract:
            failures.append(
                f"material {material_id!r} requires maps but no UV/textureProjection contract exists"
            )
        missing_maps = [
            channel
            for channel in required_maps
            if channel.endswith("Map")
            and channel not in maps
            and channel not in {"map", "normalMap"}
        ]
        if missing_maps and pbr:
            warnings.append(
                f"material {material_id!r} profile requires map slots not present in extracted PBR maps: {missing_maps}"
            )
    ...
    rigged_components = {
        ...
        for item in (rig.get("bones", []) if isinstance(rig, dict) else [])
            ...
            for index, level in enumerate(lod):
                if isinstance(level, dict):
                    ...
                    if isinstance(ids, list):
                        ...
                        if missing:
                            failures.append(
                                f"lodPlan[{index}] references missing material IDs: {missing}"
                            )
