...
def resolve_identity(
    ...
    if explicit:
        return {
            ...
            "provenance": "explicit-user-metadata",
        }
    ...
    if resolved and resolved.get("status") == "no-match":
        return {
            "status": "no-match",
            "reason": resolved.get("reason", "metadata lookup found no match"),
        }
...
def normalize_cs2_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "identity": {
            ...
            "floatRange": metadata.get(
                "floatRange", {"min": metadata.get("minFloat"), "max": metadata.get("maxFloat")}
            ),
        },
        "float": float_value,
        ...
        "approximations": {
            "float": {
                "value": float_value,
                "approximated": float_value is None,
                "confidence": 0.0 if float_value is None else 1.0,
            },
            "paintSeed": {
                "value": paint_seed,
                "approximated": paint_seed is None,
                "confidence": 0.0 if paint_seed is None else 1.0,
            },
            "hiddenRegions": {"confidence": float(hidden_confidence), "approximated": True},
            "finishStyle": {
                "value": metadata.get("finishStyle"),
                "approximated": metadata.get("finishStyle") is None,
            },
        },
    }


def enrich_manifest_with_metadata(
    manifest: dict[str, Any], resolution: dict[str, Any]
) -> dict[str, Any]:
...
def classify_map_path(path: Path) -> dict[str, Any]:
    ...
    if any(token in name for token in ("_orm", "_mrao", "_packed")):
        return {
            ...
            "path": str(path),
            "colorSpace": "linear",
            "uvOrientation": "standard",
            "packedChannels": {"roughness": "r", "metalness": "g", "ao": "b"},
        }
...
def adapt_texture_result(extraction: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    if extraction.get("status") != "ok":
        return {
            "status": "fallback",
            "textureSource": "image-only",
            "reason": extraction.get("reason", "texture extraction unavailable"),
        }
            ...
            for channel, packed_channel in record.get("packedChannels", {}).items():
                channels.setdefault(
                    channel,
                    {
                        ...
                        "source": record.get("source", "local-vpk"),
                        "derived": True,
                        "packedChannel": packed_channel,
                        "colorSpace": "linear",
                        "uvOrientation": record.get("uvOrientation", "standard"),
                    },
                )
    missing = [
        channel
        for channel in ("albedo", "normal", "roughness", "metalness")
...
def validate_route_requirements(
    manifest: dict[str, Any], *, allow_fallback: bool = False
) -> dict[str, Any]:
    result = dict(manifest)
        ...
        if not (has_camera and has_delit):
            if allow_fallback:
                ...
                result.setdefault("warnings", []).append(
                    "projection-evidence-unavailable; procedural fallback selected"
                )
            else:
