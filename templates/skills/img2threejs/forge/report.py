...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Generate a reconstruction report from the pass ledger"
    )
    ...
    opening = (
        "PIPELINE COMPLETE through optimization-pass."
        if not missing
        else "INCOMPLETE: uncleared passes: " + ", ".join(missing) + "."
    )
    emit_status(spec)
    report = (
        "\n".join(
            [
                opening,
                f"Target: {spec.get('targetName', '(unnamed)')}",
                f"Completed passes: {', '.join(completed) or '(none)'}",
                f"Current pass: {current}",
                "Review history entries: " + str(len(spec.get("reviewHistory", []))),
            ]
        )
        + "\n"
    )
