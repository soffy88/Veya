...
<div class="flex w-full max-w-3xl flex-col rounded-xl border border-emerald-500/30 bg-terminal-panel p-4 shadow-lg">
	<div class="mb-3 flex items-center gap-2 border-b border-terminal-edge pb-3">
		...
		<h3 class="text-base font-semibold text-terminal-fg">{doc.title}</h3>
		<span class="ml-auto rounded-full border border-terminal-edge px-2 py-0.5 text-xs text-terminal-dim">待确认</span>
	</div>

	<p class="mb-3 whitespace-pre-wrap text-sm leading-relaxed text-terminal-dim">
		{doc.context_analysis}
	</p>

	<ul class="mb-4 flex flex-col gap-1.5">
		{#each doc.core_features as feature, i (i)}
			<li class="flex items-start gap-2 text-sm text-terminal-fg">
	...
	<div class="flex items-center justify-end gap-2 border-t border-terminal-edge pt-3">
		{#if onReject}
			<button
				...
				onclick={onReject}
				disabled={approving}
				class="flex items-center gap-1.5 rounded-lg border border-terminal-edge px-3 py-1.5 text-sm text-terminal-dim transition hover:text-terminal-fg disabled:opacity-40"
			>
				<X class="size-3.5" /> 拒绝
			</button>
		{/if}
		<button
			...
			onclick={onApprove}
			disabled={approving}
			class="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-emerald-500 disabled:opacity-50"
		>
			{#if approving}
