...
def cmd_selftest(_: argparse.Namespace) -> None:
    ...
    for sym in ("PlanIR", "validate", "shrink_to_mus", "optimize",
                "vcg", "check_strategyproof", "LeaseManager",
    ...
    def gate_chain() -> None:
        ir = v.parse_ir({
            "version": "o1.ir/v1", "intent": "selftest",
            "vars": [{"name": "x", "type": "int", "lo": 0, "hi": 10}],
            "constraints": [{"id": "c1", "kind": "hard", "intent": "x>=1",
    ...
    def loop_chain() -> None:
        cands = [v.InterventionCandidate("restart", delta_p=0.8, cost=0.2),
...
def cmd_plan(args: argparse.Namespace) -> None:
    """多步规划薄封装: veya-loop plan --json '{"objective": "..."}'"""
    import veya_loop as v
...
def cmd_diagnose(args: argparse.Namespace) -> None:
    """因果故障诊断薄封装: veya-loop diagnose --json '{"signals": [...]}'"""
    import veya_loop as v
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="veya-loop",
                                description="Veya Loop Engine — 因果闭环控制基板")
    ...
    sp.add_argument("--json", help="multi_step_plan 输入 (JSON 对象)")
    sd = sub.add_parser("diagnose", help="因果故障诊断薄封装")
    sd.add_argument("--json", help="causal_fault_diagnose 输入 (JSON 对象)")
    return p
