...
async def test_closed_loop_failure_rate_drops(tmp_path):
    ...
    for _ in range(300):
        ...
        cfg = ClosedLoopConfig(simulate=False, execute_fn=env, rounds=1, baseline_config="degraded")
...
async def test_threat_model_quarantine_and_threshold(tmp_path):
    ...
    out = tmp_path / "reports"
    out.mkdir(parents=True, exist_ok=True)
    r = await threat_model_evolve(
        ThreatModelConfig(quarantine_threshold=0.7),
        ThreatModelInput(signals, entity="attacker-42"),
        out,
    )
...
def test_audit_emitter_full_chain(tmp_path):
    ...
    em.decide(decision={"chosen_strategy": "aggressive_repair", "utilities": {"do(x)": 0.21}})
    em.execute(
        execution={"primitive": "circuit_break", "status": "ok", "capability_nonce": "cap_abc"}
    )
