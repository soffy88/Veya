...
def test_log_prob_matches_analytic_gaussian():
    ...
    Sigma = L @ L.T
    mu = np.array([0.3, -0.4])
    mech = CholeskyMechanism(d, mean_coef=np.array([*mu, 0.0, 0.0]),
                             chol_fixed=L)   # mean_coef: [1, pa1, pa2] 的系数
    # 修正: 直接构造函数式机制
...
def test_l3_counterfactual_differs_between_cholesky_and_diagonal():
    ...
    def scm(diag_only: bool) -> ContinuousCholeskySCM:
        ...
        mech_dep = CholeskyMechanism(1, mean_fn=lambda p: np.array([1.0]),
                                     chol_fn=lambda p: np.array([[1.0]]))
        from veya_loop import ContinuousNode
        return ContinuousCholeskySCM({
            "deploy_state": ContinuousNode("deploy_state", mech_dep, []),
            "api_service": ContinuousNode("api_service", mechanism(diag_only),
                                          ["deploy_state"]),
        })
    ...
    evidence = {"deploy_state": np.array([1.0]),
                "api_service": np.array([2.1, 1.6])}
    # L3: 若当时 do(deploy_state = 0), api_service 会是什么 (锚定本次噪声)
    cf_chol = scm_chol.l3_counterfactual(evidence, {"deploy_state": [0.0]},
                                         "api_service")
    cf_diag = scm_diag.l3_counterfactual(evidence, {"deploy_state": [0.0]},
                                         "api_service")
    ...
    assert np.allclose(mech.invert(np.array([1.0]), np.array([2.1, 1.6])), u,
                       atol=1e-9)
...
def test_continuous_scm_l3_counterfactual():
    ...
    dag = nx.DiGraph()
    dag.add_edge("deploy_state", "api_service")
    scm = ContinuousCholeskySCM.fit_from_data(
        dag, {"deploy_state": pa, "api_service": tele})

    # 本次事件: deploy=1.2 时观测到联合异常
    evidence = {"deploy_state": np.array([1.2]),
                "api_service": np.array([2.1, 1.6])}
    ...
    assert np.allclose(forward_substitute(
...
def test_continuous_scm_deterministic():
    ...
    scm = ContinuousCholeskySCM.fit_from_data(
        dag, {"deploy_state": pa, "api_service": tele})
    ev = {"deploy_state": np.array([0.5]),
          "api_service": np.array([0.8, -0.6])}
...
def test_diag_floor_and_offdiag_scale_config():
    ...
    Ld = project_conditioned_lower_triangular(np.array([1.0, 2.0]), 2,
                                              diag_floor=1e-4, diag_only=True)
    assert np.allclose(Ld, np.diag(np.diag(Ld)))


def test_kappa_penalty_positive_when_ill_conditioned():
    """病态 L (对角被压扁) → κ 惩罚 > 0; 健康 L → 0。"""
    mech_sick = CholeskyMechanism(2, mean_fn=lambda p: np.zeros(2),
                                  chol_fn=lambda p: np.array([[1e-6, 0.0], [0.5, 1e-6]]))
    assert mech_sick.condition_number_proxy() > 1e3        # 代理条件数巨大
    assert mech_sick.kappa_penalty(kappa_max=50.0) > 0.0   # 超上限 → 惩罚

    mech_healthy = CholeskyMechanism(2, mean_fn=lambda p: np.zeros(2),
                                     chol_fn=lambda p: np.array([[1.0, 0.0], [0.2, 0.8]]))
...
def test_fit_with_kappa_reg_finishes_finite_loss():
    ...
    mech = CholeskyMechanism.fit_mlp(pa, x, epochs=120, hidden=8, lr=0.05,
                                     seed=3, kappa_reg=0.1, kappa_max=50.0)
...
def _hybrid_data(n=240):
    ...
    mu = np.hstack([deploy[:, :1] * 0.8, -0.5 * deploy[:, :1]]) \
        + np.hstack([0.3 * traffic, 0.2 * traffic])
...
def test_hybrid_scm_fit_abduct_cf_and_simulate():
    ...
    specs = {
        ...
        "api_service": NodeSpec("api_service", "continuous", dim=2,
                                continuous_mech="cholesky", diag_floor=1e-3,
                                offdiag_scale=0.5),
    }
    data = _hybrid_data()
    scm = fit_hybrid_scm(build_hybrid_scm(dag, specs), data, epochs=80,
                         hidden=8, kappa_reg=0.05, kappa_max=50.0)
    ...
    # 溯因回环: 证据反演 → 反事实传播
    evidence = {"deploy_state": np.array([2.0]), "traffic": np.array([0.5]),
...
def test_hybrid_scm_discrete_parent_onehot_and_cholesky_correlation():
    ...
    scm = fit_hybrid_scm(build_hybrid_scm(dag, specs), data, epochs=100,
                         hidden=8, seed=5)

    # 仿真: 固定离散父 → 样本残差相关 (cholesky L 的非对角保持)
    sim_a = scm.simulate(n_samples=400, seed=2,
                         intervened={"deploy_state": [1.0], "traffic": [0.0]})
    ...
    sim_b = scm.simulate(n_samples=400, seed=3,
                         intervened={"deploy_state": [2.0], "traffic": [0.0]})
...
def test_hybrid_scm_diagonal_node_no_residual_correlation():
    ...
    specs = {
        ...
        "traffic": NodeSpec("traffic", "continuous", dim=1, continuous_mech="diagonal"),
        "api_service": NodeSpec("api_service", "continuous", dim=2,
                                continuous_mech="diagonal"),
    }
    data = _hybrid_data(300)
    scm = fit_hybrid_scm(build_hybrid_scm(dag, specs), data, epochs=100,
                         hidden=8, seed=6)
    sim = scm.simulate(n_samples=400, seed=7,
                       intervened={"deploy_state": [1.0], "traffic": [0.0]})
...
def test_kappa_reg_auto_default_by_mechanism():
    ...
    _, _, stats_chol = fit_hybrid_scm(build_hybrid_scm(dag, specs_chol), data,
                                      epochs=30, hidden=8, return_stats=True)
    ...
    _, _, stats_diag = fit_hybrid_scm(build_hybrid_scm(dag, specs_diag), data,
                                      epochs=30, hidden=8, return_stats=True)
    assert stats_diag["kappa_reg"] == 0.0

    # 显式关闭
    _, _, stats_off = fit_hybrid_scm(build_hybrid_scm(dag, specs_chol), data,
                                     epochs=30, hidden=8, kappa_reg=0.0,
                                     return_stats=True)
...
def test_return_stats_monitoring_fields():
    ...
    _scm, losses, stats = fit_hybrid_scm(build_hybrid_scm(dag, specs), data,
                                        epochs=40, hidden=8, return_stats=True)
    assert len(losses) == 40
    assert all(np.isfinite(lv) for lv in losses)
    assert losses[-1] < losses[0]                       # 训练在下降
    assert {"kappa_reg", "min_diag", "mean_kappa_proxy",
            "mean_kappa_penalty", "mean_exact_kappa"} <= set(stats)
...
def test_cheap_proxy_catches_offdiag_pathology():
    ...
    mech = CholeskyMechanism(2, mean_fn=lambda p: np.zeros(2),
                             chol_fn=lambda p: np.array([[1e-6, 0.0], [0.5, 1e-6]]))
    proxy = mech.condition_number_proxy()
    assert proxy > 1e3                                 # 对角比=1 但非对角项放大
    assert mech.kappa_penalty(kappa_max=50.0) > 0.0
    # 健康 L: 代理小, 无惩罚
    healthy = CholeskyMechanism(2, mean_fn=lambda p: np.zeros(2),
                                chol_fn=lambda p: np.array([[1.0, 0.0], [0.2, 0.8]]))
