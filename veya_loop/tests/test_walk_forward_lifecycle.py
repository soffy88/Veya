...
def test_optimize_loop_integer_log_integration():
    ...
    r = optimize_loop(
        search_space={
            "lookback": ParamSpec(5, 60, kind="integer"),
...
def test_early_stop_rounds_marks_result():
    r_full = optimize_loop(
        search_space={"x": (0.0, 1.0)},
        evaluate=_flat_objective,
        train_window=TRAIN,
        gate_window=TEST,
        risk_gate=RiskGateConfig(),
        n_init=2,
...
def test_early_stop_ei_stop():
    ...
    res = bayesian_optimize(obj, [(0.0, 1.0)], n_init=2, n_iter=20, seed=0, ei_stop=1e-6)
...
def test_walk_forward_rejects_below_rate():
    folds = [
        (TRAIN, EvalWindow("2023-01-01", "2023-12-31", "bad")),
...
def test_lifecycle_snapshot_audit_trail():
    ...
    assert [e["event"] for e in snap["history"]] == ["promote_to_candidate", "promote_to_paper"]
