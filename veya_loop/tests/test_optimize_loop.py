...
def test_utility_default_weights_numeric():
    metrics = {"sharpe": 2.0, "total_return": 0.3, "max_drawdown": 0.1,
               "turnover": 2.0, "cost_drag": 0.2}
...
def test_fingerprint_sensitive_to_window_and_meta():
    ...
    assert fingerprint_eval({"x": 1}, TRAIN, meta={"seed": 1}) == \
           fingerprint_eval({"x": 1}, TRAIN, meta={"seed": 1})
...
def test_optimize_loop_finds_peak():
    ...
    def evaluate(params, window):
        ...
        return {"sharpe": sharpe, "total_return": 0.1, "max_drawdown": 0.1,
                "n_trades": 100, "turnover": 1.0, "cost_drag": 0.01}
...
def _window_aware_evaluate(window_label: str) -> dict[str, float]:
    if window_label == "train":
        return {"sharpe": 2.0, "total_return": 0.5, "max_drawdown": 0.05,
                "n_trades": 100, "turnover": 1.0, "cost_drag": 0.01}
    return {"sharpe": 0.1, "total_return": -0.3, "max_drawdown": 0.6,
            "n_trades": 5, "turnover": 5.0, "cost_drag": 0.2}
...
def test_optimize_loop_cache_integration():
    ...
    def evaluate(params, window):
        calls["n"] += 1
        return {"sharpe": 1.0 - abs(params["x"] - 0.5), "max_drawdown": 0.1,
                "n_trades": 50, "total_return": 0.0, "turnover": 0.0}

    cache = EvalCache()
    r1 = optimize_loop(search_space={"x": (0.0, 1.0)}, evaluate=evaluate,
                       train_window=TRAIN, gate_window=TEST,
                       risk_gate=RiskGateConfig(), cache=cache,
                       n_init=3, n_iter=6, seed=0)
    calls_after_first = calls["n"]
    # 第二次跑: 初始点/迭代点全部命中缓存 → evaluate 调用显著减少
    r2 = optimize_loop(search_space={"x": (0.0, 1.0)}, evaluate=evaluate,
                       train_window=TRAIN, gate_window=TEST,
                       risk_gate=RiskGateConfig(), cache=cache,
                       n_init=3, n_iter=6, seed=0)
