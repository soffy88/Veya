...
def _shim_modules() -> list[pathlib.Path]:
    return sorted(list((_SRC / "oprim").glob("_*.py"))
                  + list((_SRC / "omodul").glob("_*.py")))
...
def test_shim_import_all() -> None:
    """所有 shim 模块可导入且符号绑定成功 (无转发到缺失符号)。"""
    for shim in _shim_modules():
...
PROMISED_EXPORTS = {
    # P1 神经符号
    "PlanIR", "parse_ir", "validate", "compile_expr", "explain",
    "shrink_to_mus", "check_feasible", "optimize", "diff_all",
    "assign_one_to_one", "vcg", "check_strategyproof",
    "LeaseManager", "WaitForGraph", "Game", "pure_nash",
    "SnapshotStore", "MCTS", "puct", "lookahead", "ActionPlan",
    "Applier", "LocalSandbox", "SandboxPool",
    # Phase 2/3/4
    "CausalGraphStore", "causal_fault_diagnose", "closed_loop_intervene",
    "select_intervention", "expected_utility", "multi_step_plan",
    "counterfactual_rollout", "AuditEmitter",
    # L3 反事实 / 代码可靠性
    "StructuralSCM", "CholeskyMechanism", "HybridSCM",
    "bayesian_optimize", "run_code_reliability_loop",
    # 自有组件
    "HardenedExecutor", "PermissionContract", "dispatch_intervention",
    "ExecutionAdapter", "RestartAdapter", "dispatch_via_adapter",
}
