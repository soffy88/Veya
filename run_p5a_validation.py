...
import warnings, os, sys, asyncio, pathlib, shutil
...
check("hicode-headless stdin runs", r.returncode in (0, 1),
...
from oprim import make_checkpoint, restore_from_checkpoint
...
check("checkpoint: make/restore completed_steps",
...
from server.checkpoint import save_checkpoint, load_checkpoint, ckpt_path
...
check("checkpoint: file on disk", ckpt_path(TEST_SID).exists(),
      str(ckpt_path(TEST_SID)))
...
check("checkpoint: loaded step matches", state_restored.step == state_in.step,
      str(state_restored.step))
check("checkpoint: loaded completed_steps match",
...
check("checkpoint: latest-wins completed=3", len(state2.completed_steps) == 3,
...
check("coordinator: checkpoint file written", ckpt_path(COORD_SID).exists(),
      str(ckpt_path(COORD_SID)))
...
if loaded_ckpt2:
    ...
    check("coordinator: resume: resumed_from_step present",
          "resumed_from_step" in resume_result,
          str(resume_result.get("resumed_from_step")))
    check("coordinator: resume: status returned",
...
result_retry = asyncio.run(retry_with_backoff(
    flaky, max_attempts=3, base_delay=0.01,
))
...
try:
    asyncio.run(retry_with_backoff(
        fatal, max_attempts=3, base_delay=0.01,
