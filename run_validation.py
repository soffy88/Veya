...
print("=== /tool ===")
r = client.post("/tool", json={"name": "write", "args": {"path": "README.md", "content": "# hicode via /tool\nhicode.\n"}, "persona": "build"})
first = open("README.md").readline().strip()
check("/tool write disk", first == "# hicode via /tool", first)
open("README.md","w").write("# hicode\nhicode — AI 编码 agent。\n")

r = client.post("/tool", json={"name": "write", "args": {"path": "x.txt","content":"x"}, "persona": "research"})
check("/tool H2 block research", r.status_code == 403, str(r.json()))

r = client.post("/tool", json={"name": "bash", "args": {"command": "echo hi_bash"}, "persona": "build"})
...
check("/auth/key set", r.status_code==200 and os.environ.get("ANTHROPIC_API_KEY")=="sk-ant-test", str(r.json()))
check("/auth/key bad", client.post("/auth/key", json={"provider":"unknown","api_key":"x"}).status_code==400)
