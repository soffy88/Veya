...
export function ChatStream({ sessionId }: { sessionId: string | null }) {
  ...
  useEffect(() => {
    ...
    hicodeApi.getSession(sessionId).then(r => setMessages(r.messages)).catch(() => setMessages([]));
  }, [sessionId]);
  ...
  const send = async () => {
    ...
    await hicodeApi.prompt(text, sessionId ?? '', persona, model.id, model.provider);
