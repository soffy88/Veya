...
print(
    f"  status={r.status_code} cost={r.json().get('cost_usd')} content={str(r.json().get('content', ''))[:60]}",
    flush=True,
)
...
check(
    "/init: size>0",
    pathlib.Path("AGENTS.md").stat().st_size > 0 if pathlib.Path("AGENTS.md").exists() else False,
    str(pathlib.Path("AGENTS.md").stat().st_size)
    if pathlib.Path("AGENTS.md").exists()
    else "missing",
)
check(
    "/init: cost>0",
    (r.json().get("cost_usd") or 0) > 0 if r.status_code == 200 else False,
