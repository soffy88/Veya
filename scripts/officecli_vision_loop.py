...
def render(officecli: str, docx: str, out_png: str) -> subprocess.CompletedProcess:
    return subprocess.run([officecli, "render", docx, "--output", out_png],
...
def vision_check(vision_base: str, png: str) -> dict:
    ...
    payload = {
        ...
        "prompt": ("检查这份文档渲染图的排版问题: 文字溢出、元素重叠、图表遮挡、"
    ...
    return {"ok": "OK" in text.upper() and "溢出" not in text and "重叠" not in text,
            "text": text[:2000]}
...
def fix(officecli: str, docx: str, issue: str) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [officecli, "edit", docx, "--options",
         json.dumps({"note": f"vision 修复: {issue[:200]}"})],
        capture_output=True, text=True, timeout=600)
