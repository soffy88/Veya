...
def main() -> int:
    ...
    check(
        "回滚恢复基线",
        rv["reverted"] is True
        and not (wt / "broken.py").exists()
        and (wt / "app.py").read_text() == "print('v1')\n"
        and (wt / "app.py").read_text() != "print('HACKED')\n",
    )
    check(
        "坏改动隔离 (patch 含未跟踪)",
        len(rv["quarantined_changes"]) == 1
        and "HACKED" in Path(rv["quarantined_changes"][0]).read_text(),
    )
    ...
    ce = canonical_event_ingest(
        claude_output, "claude", source="verify", sink=JsonlSink(str(ev_sink))
    )
    ...
    check(
        "可回放且指纹在",
        len(replay) == 4 and all((e.get("inputs") or {}).get("fingerprint") for e in replay),
    )
    ...
    check(
        "确定性 (两次一致)",
        agent_bench_harness(tasks, ["claude"]) == agent_bench_harness(tasks, ["claude"]),
    )
    check(
        "指标齐全",
        all(
            all(k in v for k in ("completion_rate", "pass_rate", "cost"))
    ...
    evil_html = (
        '<div onclick="steal()">x</div><script>fetch("//evil")</script>'
        '<a href="javascript:alert(1)">y</a>'
    )
    ap = artifact_preview(
        {"type": "html", "content": evil_html}, snapshot_dir=str(tmp / "artifacts")
    )
    check(
        "XSS 全剥离",
        ap["renderable"]
        and "<script" not in ap["sanitized_content"].lower()
        and "onclick" not in ap["sanitized_content"]
        and "javascript:" not in ap["sanitized_content"],
    )
    ...
    bus.publish("agent.end", {"agent_id": "w1", "status": "done", "artifacts": ["tests/test_x.py"]})
    bus.publish("agent.error", {"agent_id": "w2", "error": "依赖缺失"})
    state = mon.live_state()
    check(
        "w1 投影 done+task+artifact",
        state["w1"]["status"] == "done"
        and state["w1"]["current_task"] == "写测试"
    ...
    lesson = {
        ...
        "subject_ref": "metrics",
        "lesson": "指标 API 测试失败: 时区未归一",
    }
    res = None
    for _ in range(3):
        res = sc.skill_crystallize(
            lesson, recurrence_threshold=3, lessons_file=str(tmp / "lessons.json")
        )
    check("阈值 3 次结晶", res["crystallized"] is True and res["skill_module"])
    pkg = Path(res["skill_module"]).parent
    check(
        "技能包结构 (manifest+run.py)",
        (pkg / "manifest.json").exists() and (pkg / "run.py").exists(),
    )
    ...
    check(
        "skill_hub 热载结晶技能",
        "crystallized_" in " ".join(hub.list_skills())
        or any("crystallized" in s for s in hub.list_skills()),
    ...
    b = treg.register(
        "event", "agent.end", "wf_daily_report", callback=lambda payload: fired.append(payload)
    )
    tbus.publish("agent.end", {"agent_id": "w1", "status": "done"})
    check("事件触发回调", len(fired) == 1 and fired[0]["event"] == "agent.end")
    check(
        "触发计数+持久化",
        treg.list()[0]["trigger_count"] == 1
        and TriggerRegistry(triggers_file=str(tmp / "triggers.json")).list()[0]["binding_id"]
        == b["binding_id"],
    )
