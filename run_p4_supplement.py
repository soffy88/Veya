...
check(
    "/undo: file changed on disk",
    "# CHANGED_FOR_UNDO_TEST" in changed_content,
    changed_content[:50],
)
...
check(
    "/undo: README restored to original",
...
check(
    "/compact: messages_before=12",
    result.get("messages_before") == 12,
    str(result.get("messages_before")),
)
check(
    "/compact: messages_after<12",
    (result.get("messages_after") or 99) < 12,
    str(result.get("messages_after")),
)
check(
    "/compact: chars_removed>0",
    (result.get("chars_removed") or 0) > 0,
    str(result.get("chars_removed")),
)
...
check(
    "/compact: short session skipped", r.json().get("status") == "skipped", r.json().get("status")
)
...
r = client.post(
    "/mcp/connect", json={"name": "test", "url": "http://localhost:19999/mcp", "timeout": 1.0}
)
check(
    "/mcp/connect: bad url → 500 graceful",
    r.status_code == 500,
...
print()
print(
    "  [NOTE] ④ /mcp: 待外部環境 — oprim interface verified, real MCP server needed for live tool test"
)
