"""
hicode/llm.py — canonical multi-provider LLM client

Consolidates the provider layer (previously split across ``server/providers.py``
and the stub ``llm_call``/``llm_stream`` in ``hicode/compat.py``) into a single
...
Selection order: ``config["provider"]`` > ``HICODE_LLM_PROVIDER`` env > ``dashscope``.
...
def get_provider_config(
    ...
    *,
    ...
    if not p:
        p = os.environ.get("HICODE_LLM_PROVIDER", _DEFAULT_PROVIDER)
    p = str(p).lower()
    m = model or (config or {}).get("model") or os.environ.get("HICODE_LLM_MODEL")
    if not m:
...
async def llm_call(messages: list[dict], **kwargs: Any) -> dict:
    ...
    ``HICODE_LLM_PROVIDER``/``HICODE_LLM_MODEL`` env, or defaults. Falls back to
