"""
hicode/compat.py — compatibility shims for legacy `o*` modules
...
_CHECKPOINT_DIR = pathlib.Path.home() / ".hicode" / "checkpoints"
...
async def llm_call(messages: list[dict], **kwargs: Any) -> dict:
    """
    Chat completion via the canonical LLM layer (hicode/llm.py).
    ...
    """
    from hicode.llm import llm_call as _real_llm_call
...
async def llm_stream(messages: list[dict], **kwargs: Any):
    ...
    from hicode.llm import llm_stream as _real_llm_stream
...
def permission_evaluate(action: str, resource: str | None = None, **kwargs: Any) -> dict:
    ...
    from hicode.obase.authz import evaluate_permission
...
def match_permission_rule(permissions: list, action: str, resource: str | None = None) -> bool:
    ...
    deny 命中返回 False。
    """
    from hicode.obase.authz import match_permission_rule as _match
...
# ===================================================================
# CostTracker shim (single-source → hicode.utils.CostTracker)
# ===================================================================

from hicode.utils import CostTracker as _UtilsCostTracker  # noqa: E402
