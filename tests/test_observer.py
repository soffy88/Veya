...
GOOD_CALC = CALC_BASE.replace(
    "idx = int(len(s) * p / 100)", "idx = min(len(s) - 1, int(len(s) * p / 100))"
).replace(
    "def mean(values):\n    return",
...
def test_snapshot_store_hardlink_cow(tmp_path):
    ...
    src.mkdir()
    oprim.atomic_write(str(src / "a.txt"), "hello")
    store = oprim.SnapshotStore(str(tmp_path / "store"), backend=oprim.HardlinkBackend())
...
def test_sandbox_pool_prewarm_and_reuse(tmp_path):
    pool = oprim.SandboxPool(size=3, base_dir=str(tmp_path / "pool"), isolation="netns").prewarm()
...
def test_reward_probes_and_gates(base_dir, tmp_path):
    baseline = {
        rel: (base_dir / rel).read_text(encoding="utf-8") for rel in ("calc.py", "test_calc.py")
    }
    sb = oprim.LocalSandbox(str(tmp_path), isolation="netns")
    shutil.copytree(base_dir, sb.workspace, dirs_exist_ok=True)

    probes = [
        oprim.py_syntax_gate(["calc.py"]),
...
def test_lookahead_selects_best_and_escalates(base_dir, tmp_path):
    ...
    def wp(pid, path, content):
        return oprim.ActionPlan(
            pid, [oprim.Action(f"{pid}_a", "write_file", {"path": path, "content": content})]
        )
    ...
    plans = [
        ...
        oprim.ActionPlan(
            ...
            [
                oprim.Action(
                    "a", "exec", {"cmd": "rm -rf /"}, reversibility=oprim.Reversibility.IRREVERSIBLE
                )
            ],
        ),
    ]
    probes = [
        oprim.py_syntax_gate(["calc.py", "test_calc.py"]),
    ...
    with oprim.SandboxPool(size=3, base_dir=str(tmp_path / "pool"), isolation="netns") as pool:
        v = oprim.lookahead(
            ...
            probes,
            min_reward=0.9,
            divergences=[oprim.Divergence("data_scale", "样本 5 行 vs 生产 4000 万行", "high")],
        )
    ...
    assert any(e.plan_id == "c_irr" and e.reason_code == "IRREVERSIBLE" for e in v.escalations)
