...
def test_message_body_model_partition():
    ...
    assert body2["model"] == {"providerID": "opencode-go", "modelID": "deepseek-v4-flash"}


def test_parse_sse_data():
    assert oc._parse_sse_data('data: {"type": "session.idle"}') == {"type": "session.idle"}
...
async def test_send_message_uses_cached_session(monkeypatch):
    ...
    fake_resp.json = MagicMock(return_value={"parts": [{"type": "text", "text": "回复内容"}]})
...
async def test_send_message_creates_session_when_no_cache(monkeypatch):
    ...
    fake_msg.json = MagicMock(return_value={"parts": [{"type": "text", "text": "ok"}]})
...
async def test_stream_send_yields_incremental_deltas(monkeypatch):
    ...
    class FakeClient:
        ...
        async def post(self, url, **kw):
            ...
            return MagicMock(
                json=MagicMock(return_value={"parts": [{"type": "text", "text": "我是 opencode"}]})
            )
    ...
    with patch("httpx.AsyncClient", FakeClient):
        deltas = []
        async for d in oc.stream_send("你好", "opencode-go/deepseek-v4-flash", timeout_s=5):
            deltas.append(d)
    ...
    class FakeClient:
        ...
        async def post(self, url, **kw):
            if url == "/session":
                return MagicMock(json=MagicMock(return_value={"id": sid}))
            return MagicMock(
                json=MagicMock(return_value={"parts": [{"type": "text", "text": "兜底完整回复"}]})
            )
...
def test_message_body_carries_system():
    body = oc._message_body("你好", "opencode-go/deepseek-v4-flash", system="你是 Veya")
    assert body["system"] == "你是 Veya"
    assert body["parts"] == [{"type": "text", "text": "你好"}]


def test_message_body_omits_system_when_empty():
    body = oc._message_body("你好", "opencode-go/deepseek-v4-flash", system=None)
    assert "system" not in body
    body2 = oc._message_body("你好", "opencode-go/deepseek-v4-flash", system="")
...
async def test_send_message_passes_system(monkeypatch):
    ...
    class FakeClient:
        ...
        async def post(self, url, **kw):
            ...
            return MagicMock(
                json=MagicMock(return_value={"parts": [{"type": "text", "text": "我是 Veya"}]})
            )

    monkeypatch.setattr(oc, "ensure_server", lambda timeout_s=10.0: True)
    with patch("httpx.AsyncClient", FakeClient):
        r = await oc.send_message(
            "你是谁", "opencode-go/deepseek-v4-flash", system="你是 Veya，一个全栈 AI 开发平台"
        )
...
async def test_stream_send_passes_system(monkeypatch):
    ...
    with patch("httpx.AsyncClient", FakeClient):
        got = []
        async for d in oc.stream_send(
            "你是谁", "opencode-go/deepseek-v4-flash", timeout_s=5, system="你是 Veya"
        ):
            got.append(d)
