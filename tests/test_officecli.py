...
def test_skill_hub_loads_officecli(monkeypatch):
    """skill_hub 热载三件套 (browser_use/agent_reach/officecli)。

    officecli 合法用 subprocess (它的职责就是跑 CLI 工具)——2026-08-22 起
    默认 strict, 不显式信任就会被拒载, 见 VEYA_SKILL_TRUSTED_NAMES。
    """
...
def test_officecli_write_path_whitelist():
    ...
    spec = importlib.util.spec_from_file_location("officecli_run", OFFICECLI_DIR / "run.py")
...
def test_officecli_missing_binary_structured():
    ...
    spec = importlib.util.spec_from_file_location("officecli_run2", OFFICECLI_DIR / "run.py")
    mod = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(mod)
...
def test_template_render_placeholder():
    ...
    spec = importlib.util.spec_from_file_location("officecli_run3", OFFICECLI_DIR / "run.py")
    mod = importlib.util.module_from_spec(spec)
    assert spec and spec.loader
    spec.loader.exec_module(mod)

    out = mod.render_template(
        "Hello {{name}}, period={{period}}", {"name": "Veya", "period": "W34"}
    )
...
def test_sidecar_status_shape():
    mgr = SidecarManager()
    assert mgr.status() == []
    mgr.start("probe", [sys.executable, "-c", "import time; time.sleep(60)"], health=lambda: True)
    statuses = mgr.status()
...
def test_vision_loop_dry_run():
    ...
    r = subprocess.run(
        [
            sys.executable,
            "scripts/officecli_vision_loop.py",
            "templates/office/weekly_report.md.tpl",
            "--dry-run",
        ],
        capture_output=True,
        text=True,
