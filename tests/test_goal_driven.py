...
async def test_goal_driven_loop_heartbeat_and_stall(tmp_path):
    ...
    loop = GoalDrivenLoop(
        driver, on_heartbeat=lambda gid: beats.append(gid), heartbeat_timeout_s=60
    )
...
async def test_goal_driven_api_status(tmp_path, monkeypatch):
    ...
    kernel = GoalKernel(
        AppendOnlyEventStore(str(tmp_path / "g_api.jsonl")), goal_id="g_api"
    ).rebuild()
