...
async def test_enqueue_claim_dependencies_and_idempotent_completion(tmp_path):
    ...
    try:
        ...
        duplicate = await repo.enqueue_work_item(
            WorkItemSpec(
                goal_run_id="run-1", logical_key="A", kind="agent_loop", payload={"q": "different"}
            ),
            idempotency_key="item-a",
        )
...
async def test_cross_process_claim_storm_has_one_owner_per_item(tmp_path):
    ...
    for index in range(8):
        await seed.enqueue_work_item(
            WorkItemSpec(
                goal_run_id="run-storm", logical_key=f"w-{index}", kind="agent_loop", parallel=True
            ),
        )
    await seed.close()
    ...
    try:
        claims = await asyncio.gather(
            *(repo.claim_next(f"worker-{index}") for index, repo in enumerate(workers))
        )
...
async def test_durable_claim_enforces_per_goal_parallel_limit_and_serial_isolation(tmp_path):
    ...
    try:
        ...
        await repo.create_goal_run(
            goal_run_id="run-serial", idempotency_key="run-serial", budget={"max_parallel": 4}
        )
        await repo.enqueue_work_item(
            WorkItemSpec(
                goal_run_id="run-serial", logical_key="parallel", kind="agent_loop", parallel=True
            )
        )
        await repo.enqueue_work_item(
            WorkItemSpec(
                goal_run_id="run-serial", logical_key="serial", kind="hicode", parallel=False
            )
        )
...
async def test_expired_lease_is_retried_but_stale_worker_is_fenced(tmp_path):
    ...
    try:
        ...
        await repo.enqueue_work_item(
            WorkItemSpec(
                goal_run_id="run-recovery", logical_key="safe", kind="hicode", max_attempts=2
            ),
        )
...
async def test_fanin_snapshot_and_outbox_replay_are_durable(tmp_path):
    ...
    try:
        ...
        await repo.enqueue_work_item(
            WorkItemSpec(goal_run_id="run-outbox", logical_key="A", kind="agent_loop")
        )
...
async def test_finalization_is_a_durable_idempotent_work_item(tmp_path):
    ...
    try:
        ...
        item = await repo.ensure_finalization_item(
            "run-final", snapshot_hash=snapshot["manifest_hash"]
        )
...
async def test_finalization_crash_resumes_from_checkpoint_without_child_rerun(tmp_path):
    ...
    try:
        await repo.create_goal_run(
            goal_run_id="run-final-recovery", idempotency_key="run-final-recovery"
        )
        snapshot = await repo.create_fanin_snapshot("run-final-recovery")
        await repo.ensure_finalization_item(
            "run-final-recovery", snapshot_hash=snapshot["manifest_hash"]
        )
        old = await repo.resume_finalization(
            "run-final-recovery", worker_id="finalizer-old", lease_ttl_s=1
        )
        assert old is not None
...
async def test_reconcile_committed_provider_evidence_does_not_replay_side_effect(tmp_path):
    ...
    try:
        await repo.create_goal_run(
            goal_run_id="run-provider-recovery", idempotency_key="run-provider-recovery"
        )
        item = await repo.enqueue_work_item(
            WorkItemSpec(
...
async def test_worker_host_owns_claim_start_heartbeat_and_complete(tmp_path):
    ...
    try:
        ...
        await repo.enqueue_work_item(
            WorkItemSpec(goal_run_id="run-worker", logical_key="A", kind="agent_loop")
        )
        seen: list[str] = []
...
async def test_artifact_registration_is_content_immutable(tmp_path):
    ...
    try:
        ...
        first = await repo.register_artifact(
            goal_run_id="run-artifacts",
            content_uri="outputs/report.md",
            content_hash_value="sha256:one",
            size_bytes=3,
