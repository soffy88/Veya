...
async def test_autonomous_workflow():
    ...
    # 3. 存储执行结果到记忆
    agent.store_memory(
        "Calculator 模式：使用类封装数学运算", ["calculator", "design_pattern", "implementation"]
    )
...
async def test_end_to_end_autonomous_and_collaborative_workflow():
    ...
    print("✓ Executed plan successfully")
    ...
    knowledge_items = [
        ...
        ("密码安全：使用 bcrypt 哈希，加盐", ["auth", "security", "password"]),
        ("OAuth2 流程： Authorization Code Grant", ["auth", "oauth2", "third_party"]),
    ]
    ...
    planner_task = await coordinator.create_collaboration_task(
        "Design system architecture based on autonomous plan", "planner"
    )
    ...
    executor_task = await coordinator.create_collaboration_task(
        "Implement designed architecture", "executor", dependencies=[planner_task_id]
    )
    ...
    await coordinator.complete_collaboration_task(
        executor_task_id, result="Implementation completed"
    )
...
"""
        ...
        result = await coordinator.translate_code(source_code, "python", "java")
