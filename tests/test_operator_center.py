...
C = [[2, 3, 20, 20], [3, 9, 20, 20], [20, 20, 4, 9], [20, 20, 5, 4]]
...
def simple_problem(n=4, costs=None, skills=True):
    ...
    return {
        ...
        "bids": [
            {"worker_id": f"w{i}", "task_id": f"t{j}", "cost": float(costs[i][j])}
            for i in range(n)
            for j in range(n)
        ],
    }
...
def test_hungarian_tiebreak_deterministic_and_order_independent():
    ...
    shuffled = {
        ...
        "bids": [
            {"worker_id": "w1", "task_id": "t1", "cost": 1.0},
...
def test_skill_mismatch_unassigned():
    p = {
        "tasks": [{"id": "t0", "requires_skills": ("sql",)}],
...
def test_lease_manager_three_layers():
    lm = oprim.LeaseManager(order=oprim.ResourceOrder(["cache", "db", "gpu"]), default_ttl_s=10.0)
...
def test_deadlock_risk_detected_in_pipeline():
    ...
    p = {
        ...
        "workers": [
            {"id": "w0", "capacity": {"cpu": 4}, "max_tasks": 2},
            {"id": "w1", "capacity": {"cpu": 4}, "max_tasks": 2},
        ],
        "bids": [
            {"worker_id": "w0", "task_id": "t1", "cost": 1},
...
def test_decision_id_and_replay_key_reproducible():
    p = simple_problem()
    d1 = VeyaOperatorCenter.dispatch(p, payment_rule="vcg", resource_ranking=["cache", "db", "gpu"])
    d2 = VeyaOperatorCenter.dispatch(p, payment_rule="vcg", resource_ranking=["cache", "db", "gpu"])
