...
def test_skill_crystallize_threshold(tmp_path, monkeypatch):
    ...
    r1 = skill_crystallize(lesson, recurrence_threshold=3, lessons_file=str(tmp_path / "lessons.json"))
    assert r1["crystallized"] is False and r1["count"] == 1
    r2 = skill_crystallize(lesson, recurrence_threshold=3, lessons_file=str(tmp_path / "lessons.json"))
    assert r2["count"] == 2
    r3 = skill_crystallize(lesson, recurrence_threshold=3, lessons_file=str(tmp_path / "lessons.json"))
...
def test_skill_crystallize_dedup(tmp_path, monkeypatch):
    ...
    r = None
    for _ in range(3):
        r = skill_crystallize(lesson, recurrence_threshold=3, skill_name="dupe_skill",
...
def test_trigger_event_binding_fires_callback(tmp_path):
    ...
    fired: list[dict] = []
    b = reg.register("event", "agent.end", "wf_1",
...
def test_trigger_register_entrypoint(tmp_path):
    ...
    out = trigger_register({"kind": "webhook", "spec": "/hooks/x"}, "wf_9",
                           registry=reg)
