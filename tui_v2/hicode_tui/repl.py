"""
hicode_tui.repl — F. 주 REPL 루프 + G. -p/print 모드
========================================================
...
class PrintMode:
    ...
    async def run(self, task: str) -> int:
        """0=성공, 1=실패."""
        PipeFriendly.print_log("hicode: running task...")
        ...
        if error:
            PipeFriendly.print_log(f"hicode: error: {error}")
            return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.print_mode:  # pragma: no cover
        if not args.task:  # pragma: no cover
            print("hicode -p requires a task argument", file=sys.stderr)  # pragma: no cover
            return 1  # pragma: no cover
